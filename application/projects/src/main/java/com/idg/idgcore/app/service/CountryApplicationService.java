...
import com.idg.idgcore.domain.process.IProcessConfiguration;
import com.idg.idgcore.domain.service.ICountryDomainService;
...
public class CountryApplicationService extends AbstractApplicationService implements ICountryApplicationService {
    ...
    public CountryDTO getCountryByCode(SessionContext sessionContext, CountryDTO countryDTO) throws FatalException {
        ...
        if (isAuthorized(countryDTO.getAuthorized())) {
            CountryEntity countryEntity = countryDomainService.getCountryByCode(countryDTO.getCountryCode());
            result = countryAssembler.convertEntityToDto(countryEntity);
        } else {
            MutationEntity mutationEntity = mutationsDomainService.getConfigurationByCode(countryDTO.getTaskIdentifier());
    ...
    public List<CountryDTO> getCountries(SessionContext sessionContext) throws FatalException {
        ...
        try {
            countryDTOList.addAll(countryDomainService.getCountries().stream().map(entity -> countryAssembler.convertEntityToDto(entity)).collect(Collectors.toList()));
            List<MutationEntity> unauthorizedEntities = mutationsDomainService.getUnauthorizedMutation(getTaskCode());
            ...
            countryDTOList = countryDTOList.stream().collect(Collectors.groupingBy(CountryDTO::getCountryCode, Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(CountryDTO::getRecordVersion)), Optional::get))).values().stream().collect(Collectors.toList());
            countryDTOList = countryDTOList.stream().filter(dto -> !dto.getStatus().equals(DELETED)).collect(Collectors.toList());
